...
func (m *Manifest) AddSelfDescribingSurface(kind, feature, runtime string, count int, capabilities, requiredCapabilities []string) error {
	...
	if err := engine.ValidateCapabilities(toEngineCapabilities(capabilities)); err != nil {
		return err
	}
	if err := engine.ValidateCapabilities(toEngineCapabilities(requiredCapabilities)); err != nil {
		return err
	}

...
func CaptureCoverage(d *Driver, during func() error) ([]CoverageEntry, error) {
	...
	// signatures use the bounded operation context bound to the active page.
	...
	if _, err := debugger.Enable().Do(execCtx); err != nil {
		return nil, d.redactRemoteError(fmt.Errorf("debugger.Enable: %w", err))
	}
	...
	if err := profiler.Enable().Do(execCtx); err != nil {
		return nil, d.redactRemoteError(fmt.Errorf("profiler.Enable: %w", err))
	}
	...
	if _, err := startParams.Do(execCtx); err != nil {
		return nil, d.redactRemoteError(fmt.Errorf("profiler.StartPreciseCoverage: %w", err))
	}
	...
	if err != nil {
		return nil, d.redactRemoteError(fmt.Errorf("profiler.TakePreciseCoverage: %w", err))
	}

...
func TestRuntimeJSONDynamicEvidenceRejectsUnsanitizedRawDrain(t *testing.T) {
	cases := []struct {
		name string
		edit func(*RuntimeJSONDynamicEvidenceManifest)
		want string
	}{
		{
			name: "invalid phase",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Events[0].Phase = "input now"
			},
			want: "phase is invalid",
		},
		{
			name: "out of range event kind",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				sample := firstProbeSample(manifest, "R05", "cold")
				for i := range sample.Drain.Events {
					if sample.Drain.Events[i].Name == "__gosx_canvas_event" {
						sample.Drain.Events[i].Detail["eventKind"] = float64(99)
						return
					}
				}
			},
			want: "eventKind is outside bounds",
		},
		{
			name: "unsanitized event kind",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				sample := firstProbeSample(manifest, "R05", "cold")
				for i := range sample.Drain.Events {
					if sample.Drain.Events[i].Name == "__gosx_canvas_event" {
						sample.Drain.Events[i].Detail["eventKind"] = float64(2.5)
						return
					}
				}
			},
			want: "eventKind is not an integer",
		},
		{
			name: "negative byte count",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Events[1].Detail["payloadBytes"] = float64(-1)
			},
			want: "payloadBytes is outside bounds",
		},
		{
			name: "negative arg count",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				sample := firstProbeSample(manifest, "R02", "cold")
				for i := range sample.Drain.Events {
					if sample.Drain.Events[i].Kind == "runtime-call" {
						sample.Drain.Events[i].Detail["argCount"] = float64(-1)
						return
					}
				}
			},
			want: "argCount is outside bounds",
		},
		{
			name: "over limit events",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Limits.EventLimit = 1
			},
			want: "exceeds eventLimit",
		},
		{
			name: "bad limit",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Limits.EventLimit = runtimeJSONRawDrainEventLimit + 1
			},
			want: "eventLimit is outside browser clamp",
		},
		{
			name: "bounded name",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Events[0].Name = strings.Repeat("x", runtimeJSONRawDrainMaxNameLength+1)
			},
			want: "name is not sanitized",
		},
		{
			name: "bounded detail",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Events[0].Detail["extra"] = strings.Repeat("x", runtimeJSONRawDrainMaxDetailBytes)
			},
			want: "detail exceeds byte bound",
		},
		{
			name: "unknown detail key",
			edit: func(manifest *RuntimeJSONDynamicEvidenceManifest) {
				firstProbeSample(manifest, "R02", "cold").Drain.Events[1].Detail["raw<script>"] = "bad"
			},
			want: "detail has unsanitized key",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			manifest := validRuntimeJSONDynamicEvidenceManifest(t)
			tc.edit(manifest)
			assertInvalidRuntimeJSONDynamicEvidence(t, manifest, tc.want)
		})
	}
}

...
// ValidateInventoryReady validates both the inventory's shape and its canonical
// readiness. Collection may produce a structurally valid fail-closed inventory;
// gates and artifact writers must use this stricter validation before accepting it.
func ValidateInventoryReady(inv *Inventory) error {
	if err := ValidateInventory(inv); err != nil {
		return err
	}
	if inv.Structural.Gotreesitter.Failed != 0 || len(inv.Structural.Gotreesitter.Failures) != 0 {
		return fmt.Errorf("gotreesitter parse failures must be zero: failed=%d failures=%d", inv.Structural.Gotreesitter.Failed, len(inv.Structural.Gotreesitter.Failures))
	}
	for _, group := range []struct {
		name  string
		files []SourceFile
	}{
		{name: "included", files: inv.Files.Included},
		{name: "sidecar", files: inv.Files.Sidecars},
		{name: "embedded", files: inv.Files.Embedded},
	} {
		for _, file := range group.files {
			if !file.ParseOK {
				return fmt.Errorf("%s source parser failed for %s: %s", group.name, file.Path, file.ParseError)
			}
		}
	}
	if inv.Drift.Status != "pass" {
		return fmt.Errorf("canonical drift validation failed closed: status=%q", inv.Drift.Status)
	}
	for _, ratchet := range inv.Ratchets {
		if ratchet.Status != "pass" {
			return fmt.Errorf("scope ratchet %s failed closed: status=%q", ratchet.ID, ratchet.Status)
		}
	}
	return nil
}
	...
	for _, variant := range inv.Manifest.Variants {
		...
		case "current":
			...
			if variant.SizeBytes == nil || variant.BudgetBytes == nil {
				return fmt.Errorf("current runtime variant %s requires versioned sizeBytes and budgetBytes", variant.ID)
			}

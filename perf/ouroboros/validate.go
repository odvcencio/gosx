...
func ValidateInventory(inv *Inventory) error {
	...
	if err := validateInventoryFilePaths(inv); err != nil {
		return err
	}
...
func validateInventoryFilePaths(inv *Inventory) error {
	seen := map[string]string{}
	for group, files := range map[string][]SourceFile{
		"included": inv.Files.Included,
		"sidecars": inv.Files.Sidecars,
		"embedded": inv.Files.Embedded,
	} {
		for _, file := range files {
			if !isSafeRepoRelPath(file.Path) {
				return fmt.Errorf("%s file has unsafe path %q", group, file.Path)
			}
			if prior := seen[file.Path]; prior != "" {
				return fmt.Errorf("duplicate inventory file path %s in %s and %s", file.Path, prior, group)
			}
			seen[file.Path] = group
		}
	}
	for group, files := range map[string][]ExcludedFile{
		"excluded": inv.Files.Excluded,
		"audit":    inv.Files.Audit,
	} {
		for _, file := range files {
			if !isSafeRepoRelPath(file.Path) {
				return fmt.Errorf("%s file has unsafe path %q", group, file.Path)

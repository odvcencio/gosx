...
func BuildSizeEvidenceWithOptions(opts SizeEvidenceOptions) (*SizeEvidence, error) {
	...
	if opts.Canonical || strings.TrimSpace(inventoryPath) != "" {
		source, err = BuildSourceIdentity(contextBackground(), repoRoot, inventoryPath, artifactRoot)
		if err != nil {
			return nil, err
		}
	}
...
func attributeRoutes(report *SizeEvidence, exportManifest exportEvidence, manifest *buildmanifest.Manifest, resourceManifest *LoadedResourceManifest, canonical bool) error {
	...
	for _, route := range exportManifest.routes {
		...
		for _, ref := range refs {
			assetIndex, ok := byURL[ref]
			if !ok {
				if sourcePath, hashed, resolved := manifestRefSource(report.DistDir, manifest, ref); resolved {
...
func directAssetSource(distDir string, manifest *buildmanifest.Manifest, rel string) (string, buildmanifest.HashedAsset, bool) {
	...
	bucket, file := parts[0], parts[1]
	asset, ok := manifestAssetInBucket(manifest, bucket, file)
	if !ok {
		return "", buildmanifest.HashedAsset{}, false
	}
	full, err := containedPath(filepath.Join(distDir, "assets", bucket), file)
	if err != nil {
		return "", buildmanifest.HashedAsset{}, false
	}
	return full, asset, true
...
func manifestAssetInBucket(manifest *buildmanifest.Manifest, bucket, file string) (buildmanifest.HashedAsset, bool) {
	if manifest == nil {
		return buildmanifest.HashedAsset{}, false
	}
	switch bucket {
...
func runtimeManifestAssets(manifest *buildmanifest.Manifest) []buildmanifest.HashedAsset {
	if manifest == nil {
		return nil
	}
	rt := manifest.Runtime
	return []buildmanifest.HashedAsset{
...
func BuildSourceIdentity(ctx context.Context, repoRoot, inventoryPath, artifactRoot string) (SourceIdentity, error) {
	...
	if strings.TrimSpace(inventoryPath) != "" {
		...
		if err := validateInventoryOverlayFreshOnly(ctx, root, decoded); err != nil {
			return SourceIdentity{}, err
		}
		inv = decoded
	} else {
		collected, err := Collect(ctx, CollectOptions{RepoRoot: root, ArtifactRoot: artifactRoot, Canopy: false, Git: true})
		if err != nil {
			return SourceIdentity{}, err
		}
		inv = collected
		invRef = filepath.Join(artifactRoot, "source-inventory.json")
	...
	source := SourceIdentity{
		...
		InventoryRef:                 relTo(artifactRoot, invRef),
		InventorySHA256:              hash,
		RejectsModuleCacheMismatch:   true,
		CurrentOverlayVerified:       true,
		CurrentOverlayVerificationAt: time.Now().UTC().Format(time.RFC3339),
	...
	opts := RuntimeJSONProbeOptions{
		RepoRoot:      root,
		InventoryPath: invRef,
		ArtifactRoot:  artifactRoot,
		Git:           false,
		scanMode:      runtimeJSONInventoryScanCompatibilityAnchorArchive,
	}
	source.RuntimeProbeNameCount = len(probeNames)
	source.RuntimeProbeNames = probeNames
	staticIdentity, err := collectRuntimeJSONStaticIdentity(ctx, opts, invRef)
	if err != nil {
		return SourceIdentity{}, err
	}
	if err := validateRuntimeJSONStaticIdentity(&staticIdentity); err != nil {
		return SourceIdentity{}, err
	}
	compatibilityIdentity := compatibilityAuditIdentityFromInventory(inv.Surface.CompatibilityAudit)
	if compatibilityIdentity == nil {
		return SourceIdentity{}, fmt.Errorf("canonical source identity requires compatibility audit")
	}
	if err := validateCompatibilityAuditIdentity(compatibilityIdentity); err != nil {
		return SourceIdentity{}, err
	}
	if err := requireRuntimeJSONStaticCompatibilityMatch(staticIdentity, compatibilityIdentity); err != nil {
		return SourceIdentity{}, err
	}
	source.RuntimeJSONStatic = &staticIdentity
	source.CompatibilityAudit = compatibilityIdentity
	return source, nil
}

func validateInventoryOverlayFreshOnly(ctx context.Context, repoRoot string, inv *Inventory) error {
...
func collectRuntimeJSONStaticIdentity(ctx context.Context, opts RuntimeJSONProbeOptions, inventoryPath string) (RuntimeJSONStaticIdentity, error) {
...
func requireRuntimeJSONStaticCompatibilityMatch(static RuntimeJSONStaticIdentity, audit *CompatibilityAuditIdentity) error {
	if audit == nil || audit.RuntimeJSONSemanticHash == "" || audit.RuntimeJSONCountsHash == "" || audit.RuntimeJSONGlobalNameHash == "" || audit.RuntimeJSONSourceIdentityHash == "" {
		return fmt.Errorf("compatibility audit is missing runtime JSON bindings")
	}
...
func validateRuntimeJSONStaticIdentity(identity *RuntimeJSONStaticIdentity) error {
	...
	if identity.QueryID == "" || identity.SourceIdentityHash == "" || identity.SemanticHash == "" || identity.CountsHash == "" || identity.GlobalNameHash == "" {
		return fmt.Errorf("runtime JSON static identity is incomplete")
	}
...
func validateCompatibilityAuditIdentity(identity *CompatibilityAuditIdentity) error {
	if identity == nil {
		return fmt.Errorf("compatibility audit identity is nil")
	}

...
const (
	runtimeJSONRawDrainEventLimit      = 8192
	runtimeJSONRawDrainMaxCount        = 1 << 30
	runtimeJSONRawDrainMaxNameLength   = 160
	runtimeJSONRawDrainMaxShortLength  = 32
	runtimeJSONRawDrainMaxMediumLength = 64
	runtimeJSONRawDrainMaxSourcePath   = 160
	runtimeJSONRawDrainMaxDetailKeys   = 16
	runtimeJSONRawDrainMaxDetailBytes  = 4096
	runtimeJSONRawDrainMaxArgValues    = 16
)
...
func BuildRuntimeJSONDynamicEvidence(input RuntimeJSONDynamicEvidenceInput) (*RuntimeJSONDynamicEvidenceManifest, error) {
	...
	for _, in := range input.Samples {
		...
		if errs := validateRuntimeJSONRawDrain(sample, manifest.Static); len(errs) > 0 {
			err := errors.New(strings.Join(uniqueStrings(errs), "; "))
...
func validateRuntimeJSONDynamicProbeSample(sample RuntimeJSONDynamicSample, static RuntimeJSONDynamicStaticBinding) []string {
	...
	errs = append(errs, validateRuntimeJSONRawDrain(sample, static)...)
...
func validateRuntimeJSONRawDrain(sample RuntimeJSONDynamicSample, static RuntimeJSONDynamicStaticBinding) []string {
	drain := sample.Drain
	if drain == nil {
		return nil
	}
	var errs []string
	prefix := sample.ID + ": raw drain "
	if drain.Limits.EventLimit <= 0 || drain.Limits.EventLimit > runtimeJSONRawDrainEventLimit {
		errs = append(errs, prefix+"eventLimit is outside browser clamp")
	}
	if len(drain.Events) > drain.Limits.EventLimit {
		errs = append(errs, fmt.Sprintf("%sevents = %d exceeds eventLimit %d", prefix, len(drain.Events), drain.Limits.EventLimit))
	}
	if !runtimeJSONRawPhaseAllowed(drain.Phase) {
		errs = append(errs, prefix+"phase is invalid: "+drain.Phase)
	}
	if !runtimeJSONRawCleanString(drain.RouteID, runtimeJSONRawDrainMaxShortLength) {
		errs = append(errs, prefix+"routeID is not sanitized")
	}
	for _, group := range []struct {
		label  string
		values []string
	}{
		{"wrappedGlobals", drain.WrappedGlobals},
		{"unwrappedGlobals", drain.UnwrappedGlobals},
		{"knownGlobals", drain.KnownGlobals},
	} {
		for _, value := range group.values {
			if !runtimeJSONRawCleanString(value, runtimeJSONRawDrainMaxNameLength) {
				errs = append(errs, fmt.Sprintf("%s%s contains unsanitized name %q", prefix, group.label, value))
			}
		}
	}
	for i, event := range drain.Events {
		errs = append(errs, validateRuntimeJSONRawProbeEvent(sample, static, event, i)...)
	}
	return errs
}

func validateRuntimeJSONRawProbeEvent(sample RuntimeJSONDynamicSample, static RuntimeJSONDynamicStaticBinding, event ProbeEvent, index int) []string {
	prefix := fmt.Sprintf("%s: raw event %d ", sample.ID, index)
	var errs []string
	switch event.Kind {
	case "probe", "json-call", "runtime-call":
	default:
		errs = append(errs, prefix+"has invalid kind: "+event.Kind)
	}
	if !runtimeJSONRawCleanString(event.Kind, runtimeJSONRawDrainMaxShortLength) {
		errs = append(errs, prefix+"kind is not sanitized")
	}
	if !runtimeJSONRawCleanString(event.Name, runtimeJSONRawDrainMaxNameLength) {
		errs = append(errs, prefix+"name is not sanitized")
	}
	if !runtimeJSONRawPhaseAllowed(event.Phase) {
		errs = append(errs, prefix+"phase is invalid: "+event.Phase)
	}
	if event.StartTime < 0 {
		errs = append(errs, prefix+"startTime is negative")
	}
	if event.Detail == nil {
		errs = append(errs, prefix+"detail is missing")
		return errs
	}
	if len(event.Detail) > runtimeJSONRawDrainMaxDetailKeys {
		errs = append(errs, prefix+"detail has too many keys")
	}
	if body, err := json.Marshal(event.Detail); err != nil {
		errs = append(errs, prefix+"detail is not JSON serializable")
	} else if len(body) > runtimeJSONRawDrainMaxDetailBytes {
		errs = append(errs, prefix+"detail exceeds byte bound")
	}
	errs = append(errs, validateRuntimeJSONRawDetail(event.Kind, event.Detail, static, prefix)...)
	return errs
}

func validateRuntimeJSONRawDetail(kind string, detail map[string]any, static RuntimeJSONDynamicStaticBinding, prefix string) []string {
	var errs []string
	allowed := map[string]bool{"routeID": true}
	switch kind {
	case "json-call":
		for _, key := range []string{"operation", "payloadBytes", "resultBytes", "exception", "stackHash", "source"} {
			allowed[key] = true
		}
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "operation", runtimeJSONRawDrainMaxShortLength, prefix)...)
		errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, "payloadBytes", 0, runtimeJSONRawDrainMaxCount, prefix)...)
		errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, "resultBytes", 0, runtimeJSONRawDrainMaxCount, prefix)...)
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "exception", runtimeJSONRawDrainMaxMediumLength, prefix)...)
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "stackHash", runtimeJSONRawDrainMaxShortLength, prefix)...)
		errs = append(errs, validateRuntimeJSONRawSource(detail, prefix)...)
	case "runtime-call":
		for _, key := range []string{"argCount", "argTypes", "argBytes", "resultType", "resultBytes", "exception", "async", "stackHash", "source", "eventKind"} {
			allowed[key] = true
		}
		errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, "argCount", 0, runtimeJSONRawDrainMaxArgValues, prefix)...)
		errs = append(errs, validateRuntimeJSONRawStringArray(detail, "argTypes", runtimeJSONRawDrainMaxArgValues, runtimeJSONRawDrainMaxShortLength, prefix)...)
		errs = append(errs, validateRuntimeJSONRawIntArray(detail, "argBytes", runtimeJSONRawDrainMaxArgValues, 0, runtimeJSONRawDrainMaxCount, prefix)...)
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "resultType", runtimeJSONRawDrainMaxShortLength, prefix)...)
		errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, "resultBytes", 0, runtimeJSONRawDrainMaxCount, prefix)...)
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "exception", runtimeJSONRawDrainMaxMediumLength, prefix)...)
		errs = append(errs, requireRuntimeJSONRawBool(detail, "async", prefix)...)
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "stackHash", runtimeJSONRawDrainMaxShortLength, prefix)...)
		errs = append(errs, validateRuntimeJSONRawSource(detail, prefix)...)
		if _, ok := detail["eventKind"]; ok {
			errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, "eventKind", 1, 5, prefix)...)
		}
		if name, ok := detail["routeID"].(string); ok && name != "" && !canonicalRouteIDSet()[name] {
			errs = append(errs, prefix+"routeID has unknown canonical route")
		}
	case "probe":
		for _, key := range []string{"knownGlobalCount", "wrappedCount", "unwrappedCount", "nameHash", "phase"} {
			allowed[key] = true
		}
		for _, key := range []string{"knownGlobalCount", "wrappedCount", "unwrappedCount"} {
			if _, ok := detail[key]; ok {
				errs = append(errs, requireRuntimeJSONRawBoundedInt(detail, key, 0, runtimeJSONRawDrainMaxCount, prefix)...)
			}
		}
		if _, ok := detail["nameHash"]; ok {
			errs = append(errs, requireRuntimeJSONRawCleanString(detail, "nameHash", runtimeJSONRawDrainMaxShortLength, prefix)...)
		}
		if phase, ok := detail["phase"].(string); ok && !runtimeJSONRawPhaseAllowed(phase) {
			errs = append(errs, prefix+"detail phase is invalid")
		}
	default:
	}
	for key := range detail {
		if !allowed[key] {
			errs = append(errs, prefix+"detail has unsanitized key: "+key)
		}
	}
	if _, ok := detail["routeID"]; ok {
		errs = append(errs, requireRuntimeJSONRawCleanString(detail, "routeID", runtimeJSONRawDrainMaxShortLength, prefix)...)
	}
	return errs
}

func validateRuntimeJSONRawSource(detail map[string]any, prefix string) []string {
	source, ok := detail["source"].(map[string]any)
	if !ok {
		return []string{prefix + "source is missing"}
	}
	allowed := map[string]bool{"urlHash": true, "path": true, "line": true, "column": true}
	var errs []string
	for key := range source {
		if !allowed[key] {
			errs = append(errs, prefix+"source has unsanitized key: "+key)
		}
	}
	errs = append(errs, requireRuntimeJSONRawCleanString(source, "urlHash", runtimeJSONRawDrainMaxShortLength, prefix)...)
	errs = append(errs, requireRuntimeJSONRawCleanString(source, "path", runtimeJSONRawDrainMaxSourcePath, prefix)...)
	errs = append(errs, requireRuntimeJSONRawBoundedInt(source, "line", 0, runtimeJSONRawDrainMaxCount, prefix)...)
	errs = append(errs, requireRuntimeJSONRawBoundedInt(source, "column", 0, runtimeJSONRawDrainMaxCount, prefix)...)
	return errs
}

func runtimeJSONRawPhaseAllowed(phase string) bool {
	if phase == "unknown" {
		return true
	}
...
func runtimeJSONRawCleanString(value string, max int) bool {
	if len(value) > max {
		return false
	}
	return value == runtimeJSONRawCleanName(value, max)
}

func runtimeJSONRawCleanName(value string, max int) string {
	if max <= 0 {
		max = 120
	}
...
func requireRuntimeJSONRawCleanString(detail map[string]any, key string, max int, prefix string) []string {
	value, ok := detail[key]
	if !ok {
		return []string{prefix + key + " is missing"}
	}
	s, ok := value.(string)
	if !ok {
		return []string{prefix + key + " is not a string"}
	}
	if !runtimeJSONRawCleanString(s, max) {
		return []string{prefix + key + " is not sanitized"}
	}
	return nil
}

func requireRuntimeJSONRawBool(detail map[string]any, key string, prefix string) []string {
	if value, ok := detail[key]; ok {
		if _, ok := value.(bool); !ok {
			return []string{prefix + key + " is not boolean"}
		}
	}
	return nil
}

func requireRuntimeJSONRawBoundedInt(detail map[string]any, key string, min, max int, prefix string) []string {
	value, ok := detail[key]
	if !ok {
		return []string{prefix + key + " is missing"}
	}
	n, ok := runtimeJSONRawInt(value)
	if !ok {
		return []string{prefix + key + " is not an integer"}
	}
	if n < min || n > max {
		return []string{prefix + key + " is outside bounds"}
	}
	return nil
}

func validateRuntimeJSONRawStringArray(detail map[string]any, key string, maxLen, maxString int, prefix string) []string {
	value, ok := detail[key]
	if !ok {
		return []string{prefix + key + " is missing"}
	}
	values, ok := value.([]any)
	if !ok {
		return []string{prefix + key + " is not an array"}
	}
	var errs []string
	if len(values) > maxLen {
		errs = append(errs, prefix+key+" exceeds item bound")
	}
	for _, value := range values {
		s, ok := value.(string)
		if !ok || !runtimeJSONRawCleanString(s, maxString) {
			errs = append(errs, prefix+key+" contains unsanitized string")
			break
		}
	}
	return errs
...
func validateRuntimeJSONRawIntArray(detail map[string]any, key string, maxLen, min, max int, prefix string) []string {
	value, ok := detail[key]
	if !ok {
		return []string{prefix + key + " is missing"}
	}
	...
	for _, value := range values {
		n, ok := runtimeJSONRawInt(value)
		if !ok || n < min || n > max {
			errs = append(errs, prefix+key+" contains out-of-bounds integer")
			break
		}
	}
	return errs
...
func runtimeJSONRawInt(value any) (int, bool) {
	switch v := value.(type) {
	case int:
		return v, true
	case int64:

...
func CompareOuroborosArtifacts(opts CompareOptions) (*CompareReport, error) {
	...
	if opts.RejectSelfCompare && report.SelfCompare {
		report.addCheck("compat.baseline.distinct", "compatibility", "fail", "baseline and candidate resolve to the same evidence; compare against a committed versioned baseline")
	}
...
func metricApplicable(mode string, baseline, candidate *compareLoadedArtifact, bucket metricBucket, metricID string) (bool, string) {
	...
	case metricID == "memory.wasmPages" || metricID == "memory.wasmBytes":
		...
		if mode == CompareModeSmoke &&
			!bucketHasDerivedMetric(baseline, bucket, metricID) &&
			!bucketHasDerivedMetric(candidate, bucket, metricID) {
			return false, "WASM memory telemetry unavailable in smoke"
		}
	}
	return true, ""
}
...
func bucketHasDerivedMetric(artifact *compareLoadedArtifact, bucket metricBucket, metricID string) bool {
	if artifact == nil {
		return false
	}
	for _, sample := range artifact.raw {
		...
		if _, ok := deriveSampleMetrics(sample)[metricID]; ok {
			return true
		}
	}
	return false
...
func routeSelectsWASM(artifact *compareLoadedArtifact, routeID string) bool {
	for _, route := range artifact.manifest.Corpus.Routes {
		...
		switch strings.ToLower(strings.TrimSpace(route.ExpectedTinyGoCurrent)) {
		case "none":
			return false
		case "":
			// Fall through to the inventory selection when the route manifest
			// does not make an explicit current-runtime assertion.
		default:
			return true
		}
	}

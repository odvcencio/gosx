...
setInterval(() => {
  ...
  m.setAttribute("data-gosx-scene3d-webgl-frame-seq", String(Number(m.getAttribute("data-gosx-scene3d-webgl-frame-seq") || 0) + 1));
	...
	if _, err := os.Stat(filepath.Join(manifest.ArtifactRoot, "pixel-evidence.json")); !os.IsNotExist(err) {
		t.Fatalf("canonical manifest exists after synthetic failure: %v", err)
	}
	assertNoPixelStagingRoots(t, manifest.ArtifactRoot)
...
func assertNoPixelStagingRoots(t *testing.T, finalRoot string) {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(filepath.Dir(finalRoot), "."+filepath.Base(finalRoot)+".staging-*"))
	if err != nil {
		t.Fatalf("glob staging roots: %v", err)
	}
	if len(matches) != 0 {
		t.Fatalf("staging roots remain: %v", matches)
	}
}

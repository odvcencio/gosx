...
func TestRaycastCoverageManifest(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			nativeHit:   true,
			browserPick: false,
			note:        "sceneRaycastPick never reads bundle.points, so the browser reports no hit",
		},
		{
			name:        "sprite",
			...
			nativeHit:   true,
			browserPick: false,
			note:        "a sprite lowers to a DOM overlay and the browser picks it through pointer events",
		},
		{

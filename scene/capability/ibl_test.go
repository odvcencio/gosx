...
// TestWebGL2GatesIBLOnNineteenUnits corroborates the false WebGL2 cell WITH
...
func TestWebGL2GatesIBLOnNineteenUnits(t *testing.T) {
	...
	missingGate := missingSymbols(webglRendererPath, webgl,
		"maxUnits >= 19", "scenePBRHDRIBLAvailable",
	)
	if len(missingGate) > 0 {
		t.Fatalf("the 19 fragment-texture-unit gate moved or was renamed; update this test and the "+
	...
	if Matrix[FeatureIBL][BackendWebGL] {
		t.Fatal("Matrix[ibl][webgl] is true, but the WebGL2 consumer only activates at " +
			">= 19 fragment texture units (scenePBRHDRIBLAvailable). An unconditional cell " +
...
func TestWebGL2IBLGateHasSHFallbackReason(t *testing.T) {
	...
	if strings.Contains(webgl, "sh-irradiance") && !strings.Contains(webgl, "fragment-texture-units<19 -> sh-irradiance") {
		t.Fatal(`found a "sh-irradiance" reason string that does not match the pinned spelling ` +
			`"fragment-texture-units<19 -> sh-irradiance"; keep the diagnostics reason stable so a ` +

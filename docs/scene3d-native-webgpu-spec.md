...
Stable-membership Go/WASM clients may call `scene.PackInstancedGLBPoseFrame` and submit the resulting bytes through the mounted Scene3D handle's synchronous `applyInstancedGLBPoseFrame` method when the handle advertises `__gosxScene3DInstancedGLBPoseReady`. The packet carries the exact last full InstancedGLB declaration revision and complete ordered ID layout. The runtime validates every transform, current wrapper, hydration owner, and animation row before committing any pose. A rejected packet consumes no revision, so the caller must send the same update through the ordinary JSON command path. Membership, source, material, clip, loop, parent-matrix, and other declaration changes always use that full path.

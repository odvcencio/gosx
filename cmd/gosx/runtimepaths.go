...
func resolveGoSXModuleRootWithFlags(projectDir, moduleFlags string) (string, error) {
	out, err := goListDirWithFlags(projectDir, gosxModuleImportPath, moduleFlags)
	if err != nil {
		return "", fmt.Errorf("resolve %s module root: %w", gosxModuleImportPath, err)
	}
	if out == "" {
...
func goListDirWithFlags(projectDir, importPath, moduleFlags string) (string, error) {
	cmd := exec.Command("go", "list", "-f", "{{.Dir}}", importPath)
	cmd.Dir = projectDir

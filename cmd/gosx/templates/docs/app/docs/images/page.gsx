...
func Page() Node {
	return <article class="prose">
		...
		<h1>Image handling now covers the production chores instead of leaving raw files untouched.</h1>
		<p>
		...
		<section class="feature-grid">
			<div class="card">
				...
				<p>The same source asset can emit multiple widths and let the browser choose the right one.</p>
			</div>
			<div class="card">
				<strong>Server-side resize</strong>
				<p>PNG, JPEG, and GIF sources can be resized before the bytes hit the client.</p>
			</div>
			<div class="card">
				<strong>Safe defaults</strong>
				<p>Root-relative local paths optimize. Remote URLs and SVGs fall back to a plain img tag.</p>
			</div>
			<div class="card">

// encode.go translates render/gpu descriptor structs into the WebGPU
...
//
// Nothing in this file touches syscall/js. It carries no build tag for that
// reason: the mapping tables are the part of the backend most likely to hold a
// silent typo, and a wrong format string breaks the whole desktop WebGPU
// binding at run time with no compiler complaint. Without the tag the tables
// compile and test on every platform. encode_js.go keeps the one function that
// does reach into a js.Value.
package jsgpu

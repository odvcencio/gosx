...
// dashboardConversionGolden is the dashboard root page's canonical rendered
// output after the compiler-owned GSX layout policy removes source-only
// inter-element whitespace, with the footer's live clock normalized to TIME.
//
// This is the task's required proof: not that app/page.gsx compiles as
// `component Page(props: PageProps)`, but that it renders the same intended
// HTML once converted, reading the same Load hook's data through typed,
...
// TestDashboardRootPageStrictPropsMatchCanonicalHTML renders app/'s converted
// strict Page entry through the real Router.AddDir file-routing path (the
// same path main() builds) and asserts the response body, with the footer's
// live clock normalized, matches dashboardConversionGolden.
func TestDashboardRootPageStrictPropsMatchCanonicalHTML(t *testing.T) {
	...
	if got != dashboardConversionGolden {
		t.Fatalf("rendered body does not match the canonical HTML golden.\ngot:  %s\nwant: %s", got, dashboardConversionGolden)
	}
}

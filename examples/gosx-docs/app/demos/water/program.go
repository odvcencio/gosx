...
var waterObjectMaterialSelenaUniforms = map[string]any{
	...
	"poolWidth":       1.0,
	"poolLength":      1.0,
	"baseColor":       []float64{0.78, 0.42, 0.16, 1},
	"isTexturePass":   0.0,
	"texturePassMode": 0.0,
	"lightDir":        []float64{1.4, 2.8, -1.6},
	"grid":            256.0,
	...
	payload := map[string]any{
		...
		"objects": map[string]any{
			...
			"Rubber Duck": map[string]any{
				...
				"objectSubtype": "duck",
				"objectX":       0.4,
				// Keep the low-density rubber duck at its natural buoyancy
				// line. Object switches preserve the previous object's shared
				// position, so floorClearance must clamp a floor-resting
				// sphere/cube up to this line rather than leaving the entire
				// textured duck hidden under the surface.
				"objectY":                   -0.1,
				"objectZ":                   -0.2,
				"objectRadius":              0.32,
				"objectHalfSizeX":           0,
				...
				"mesh": map[string]any{
					...
					"bounds": 0.65, "fit": "contain", "fitAlign": "center-min-y",

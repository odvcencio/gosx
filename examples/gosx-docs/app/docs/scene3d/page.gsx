...
func Page() Node {
	return <div>
		<section id="scene-graph">
			<h2>Scene Graph</h2>
			<p>
				GoSX ships a ground-up PBR renderer — not a wrapper around Three.js or Babylon.js.
				Scenes are declared in pure Go using typed structs. The server serialises the scene
				graph into a compact wire format and a browser-side engine reconstructs it on WebGPU
				or WebGL2 at runtime. No JavaScript scene code ever touches your server.
			</p>
			<p>
				A scene starts with <span class="inline-code">scene.Props</span>, which describes
				the canvas dimensions, background, camera, environment, and the root
...
props := scene.Props{
    ...
    Graph: scene.NewGraph(
        // lights, meshes, models, particles ...
			...
			<p>
				Pass the props to the <span class="inline-code">Scene3D</span> component from
				your route loader:
			</p>
    ...
    return map[string]any{
        "scene": MyScene(),
			...
			<p>
				The live demo above is a full PBR scene declared in Go and rendered by the engine.
		...
		<section id="camera-controls">
			...
			<p>
				The <span class="inline-code">scene.PerspectiveCamera</span> struct sets the
				initial view. <span class="inline-code">Position</span> and
				<span class="inline-code">Rotation</span> accept
				<span class="inline-code">scene.Vec3</span> and
				<span class="inline-code">scene.Euler</span> helpers.
			</p>
    ...
    Far:      1000,
			...
			<p>
				The <span class="inline-code">Controls</span> field accepts
				<span class="inline-code">"orbit"</span> for standard drag-to-orbit behaviour.
				Omit it for a static camera. The engine handles pointer, touch, and wheel events
				natively.
			</p>
...
ScrollCameraEnd:   1.0,  // lerp end (1 = bottom of page)`)}
			<p>
				<span class="inline-code">DragToRotate</span> restricts orbit to pointer-drag
				only, which is useful for full-page heroes where scroll must not conflict with
				page scrolling.
			</p>
		</section>
		...
		<section id="geometries">
			<h2>Geometries</h2>
			<p>
				All eight primitives implement the <span class="inline-code">scene.Geometry</span>
				interface. Pass any of them as the <span class="inline-code">Geometry</span> field
				of a <span class="inline-code">scene.Mesh</span>.
			</p>
			<div class="scene3d-geometry-grid">
				...
				<div class="scene3d-geometry-card glass-panel">
					...
					<span class="scene3d-geometry-fields">RadiusTop, RadiusBottom, Height, Segments</span>
				</div>
				<div class="scene3d-geometry-card glass-panel">
				...
				<div class="scene3d-geometry-card glass-panel">
					...
					<span class="scene3d-geometry-fields">Radius, Tube, RadialSegments, TubularSegments</span>
				</div>
				<div class="scene3d-geometry-card glass-panel">
...
scene.Mesh{
    ...
    Material: scene.StandardMaterial{Color: "#C0C0C0", Roughness: 0.3, Metalness: 0.8},
		...
		<section id="materials">
			<h2>Materials</h2>
			<p>
				<span class="inline-code">scene.StandardMaterial</span> implements the
				physically-based roughness/metalness workflow. Every parameter maps directly
				to the PBR model running in the GPU shader.
			</p>
...
scene.MatteMaterial{Color: "#1a1a18"}`)}
		...
		<section id="lights-shadows">
			...
			<p>
				Five light types cover the full photometric range. All lights are added as
				nodes to <span class="inline-code">scene.NewGraph()</span>.
			</p>
...
scene.HemisphereLight{
    ...
    Intensity:   0.4,
			...
			<p>
				Shadow maps are generated per <span class="inline-code">DirectionalLight</span>
				and <span class="inline-code">SpotLight</span> with
				<span class="inline-code">CastShadow: true</span>.
				Individual meshes opt in with <span class="inline-code">CastShadow</span> and
		...
		<section id="animation">
			<h2>Animation</h2>
			<p>
				Procedural animation is declared on meshes — no keyframe timeline required.
				The engine drives it in the GPU update loop.
			</p>
    ...
    Loop:      scene.Bool(true),
		...
		<section id="particles">
			<h2>Particles</h2>
			<p>
				<span class="inline-code">scene.ComputeParticles</span> drives a GPU compute
				particle system. On WebGPU the full simulation runs in WGSL compute shaders;
				on WebGL2 it falls back to a CPU-updated vertex buffer. Both paths produce
				the same visual at 100 K+ particles.
			</p>
    ...
    Bounds: 12,
		...
		<section id="gltf-loading">
			<h2>glTF Loading</h2>
			<p>
				<span class="inline-code">scene.Model</span> instances a framework-owned glTF
				asset. The engine loads the <span class="inline-code">.glb</span> file at
				runtime, applies any material override from Go, and attaches the model to the
				scene graph at the declared transform.
			</p>
			{CodeBlock("go", `scene.Model{
...
scene.Model{
    ...
    Position:  scene.Vec3(-2, 0, 0),
			...
			<p>
				Model assets are served from the <span class="inline-code">public/models/</span>
				directory. The engine handles binary glTF, embedded textures, and Draco-compressed
		...
		<section id="instancing">
			<h2>Instancing</h2>
			<p>
				<span class="inline-code">scene.InstancedMesh</span> renders N copies of a
				single geometry with per-instance transforms in a single draw call. Use it for
				particle fields, foliage, crowds, or any repeated geometry where instancing
				yields order-of-magnitude GPU savings.
			</p>
...
scene.InstancedMesh{
    ...
    ReceiveShadow: true,
			...
			<p>
				Per-instance rotations and scales are optional. If omitted, all instances
				share the identity rotation and a uniform scale of 1.
			</p>
		</section>
	</div>
}

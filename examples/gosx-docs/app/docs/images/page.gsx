...
/>`)}
    ...
    Widths: []int{320, 640, 960},
		...
		<section id="responsive-images">
			...
			<p>
				...
				produces a
				<span class="inline-code">srcset</span>
				attribute with one entry per requested width.
				The browser selects the most appropriate variant based on the current viewport
				and the provided
				<span class="inline-code">sizes</span>
				hint.
			</p>
    ...
    Sizes:  "(max-width: 768px) 100vw, 50vw",
			...
			<p>
				...
				references the
				<span class="inline-code">/_gosx/image</span>
				endpoint for each width, so the original full-size asset is never sent to clients
				that do not need it.
			</p>
			{CodeBlock("html", `<img
...
/>`)}
		...
		<section id="format-conversion">
			...
			<p>
				The
				<span class="inline-code">fmt</span>
				query parameter requests a specific output format.
				PNG, JPEG, and GIF sources are supported as inputs.
				WebP is the recommended output format for browsers that support it.
			</p>
...
url := server.ImageURL(server.ImageURLProps{
    ...
    Format: "webp",
			...
			<p>
				Remote URLs and SVG sources are passed through unchanged. The optimizer
				only processes root-relative paths that resolve to files in the
				<span class="inline-code">public/</span>
				directory.
			</p>
		</section>
		...
		<section id="caching">
			...
			<p>
				...
				<span class="inline-code">Cache-Control: public, immutable, max-age=31536000</span>
				by default. The URL includes the source path, dimensions, and format, so
				cache keys are content-addressed: a change to the source file at a new path
				produces a new URL with no stale responses.
			</p>
			<p>
				These URLs are CDN-friendly. When a CDN sits in front of the origin, the first
				request for each variant is forwarded and cached at the edge. Subsequent requests
				are served from cache without hitting the Go server.
			</p>
...
Vary: Accept`)}
			<div class="callout">
				...
				<p>
					In development the immutable header is omitted so refreshing the browser always
					fetches a fresh variant while you iterate on source assets.
				</p>
			</div>
		</section>
	</div>

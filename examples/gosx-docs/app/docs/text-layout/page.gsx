...
func Page() Node {
	return <div>
		<section id="textblock">
			...
			<p>
				...
				is GoSX's server-side text measurement primitive. It measures the given text against actual font metrics on the server, computes line breaks, and emits final wrapped HTML before the first byte reaches the browser. No JavaScript, no layout shift, no guessing.
			</p>
			<p>
				No other web framework does this. CSS wrapping happens in the browser after fonts load — GoSX resolves it on the server with the same font data that would produce the identical result in a canvas context.
			</p>
		...
		<section id="font-metrics">
			<h2>Font Metrics</h2>
			<p>
				GoSX ships a font measurement engine that parses TrueType and OpenType metrics without CGo. Pass a CSS font shorthand — weight, size, and family — and the engine resolves advance widths, kerning pairs, and line height to produce character-accurate line breaks.
			</p>
			<p>
				The
	...
	server.TextBlockProps{Font: "600 18px JetBrains Mono"}`)}
			<p>
				Font files referenced by loaded pages are measured once at startup and cached. Subsequent requests pay zero I/O cost for the same font.
			</p>
		</section>
		<section id="width-constraints">
			...
			<p>
				...
				is specified in pixels. The engine breaks lines at word boundaries so that no rendered line exceeds that width. Subpixel widths are handled correctly — the engine rounds to the nearest pixel boundary rather than truncating.
			</p>
			<p>
				The same text at three different widths produces three different line structures, all measured on the server:
			</p>
	...
	server.TextBlock(server.TextBlockProps{Font: "400 16px Inter", MaxWidth: 160},
	    "GoSX measures text server-side with actual font metrics.")`)}
			<p>
				When
				<span class="inline-code">MaxWidth</span>
				is zero, the text renders as a single unwrapped line, matching browser behavior for elements without a width constraint.
			</p>
		</section>
	    ...
	    Overflow: textlayout.OverflowEllipsis,
			...
			<p>
				The ellipsis is placed at a character-accurate boundary, not a CSS approximation. The server knows the exact advance width of every character and places the
				<span class="inline-code">…</span>
		...
		<section id="line-breaking">
			<h2>Line Breaking</h2>
			<p>
				The default break strategy wraps at word boundaries (spaces and soft-wrap opportunities). You can control whitespace handling with the
		...
		<section id="bootstrap-mode">
			...
			<p>
				...
				mode: the server owns the final layout and nothing runs in the browser. For cases where exact browser font metrics are required — subpixel hinting, variable fonts, or OS-level rendering differences —
				<span class="inline-code">bootstrap</span>
				mode emits a server-first stable pass and schedules a lightweight client refinement via the shared bootstrap runtime.
			</p>
	...
	server.TextBlock(server.TextBlockProps{
	    ...
	    MaxWidth: 400,
			...
			<p>
				...
				use
				<span class="inline-code">native</span>
				when server-measured layout is sufficient and you want the page to remain completely JS-free. Use
				<span class="inline-code">bootstrap</span>
				only when browser-specific font rendering produces a meaningfully different result and that difference matters for the design.
			</p>
			<section class="callout">
				<strong>No other framework does this</strong>
				<p>
					Every other server rendering framework delegates text layout to the browser. GoSX resolves wrap points, ellipsis positions, and line counts on the server using the same font data a browser would use. The HTML you ship is the final layout, not a placeholder.
				</p>
			</section>
		</section>
	</div>

...
func Page() Node {
	return <div>
		<section id="signal-basics">
			...
			<p>
				A signal is a reactive cell — a value with a subscriber list. When the value
				changes, every subscriber is notified and re-evaluates. Signals are the atomic
				unit of reactivity in GoSX; there is no framework-owned state tree, no context
				providers, and no prop threading.
			</p>
...
var ro signal.ReadOnly[bool] = active.ReadOnly()`)}
		...
		<section id="computed-values">
			...
			<p>
				A computed value derives from one or more signals. It re-evaluates lazily when
				any dependency changes and caches its result until the next change. Computeds
				are themselves read-only signals, so they can be used anywhere a signal value
				is expected.
			</p>
			{CodeBlock("go", `import "github.com/odvcencio/gosx/signal"
...
shown := visible.Get()`)}
			<p>
				Dependencies are tracked automatically. The computed function runs inside a
				tracking scope. Every
				<span class="inline-code">Get()</span>
				call during that run registers the caller as a subscriber of that signal.
				No dependency array, no manual
		...
		<section id="effects">
			...
			<p>
				An effect is a side-effecting function that runs whenever its signal dependencies
				change. Effects are scheduled after all signal writes in a batch have propagated,
				so they always see a consistent state snapshot.
			</p>
			{CodeBlock("go", `import "github.com/odvcencio/gosx/signal"
...
stop()`)}
			<p>
				Effects are primarily useful in server-side lifecycle code and in engine programs.
				In island templates, DOM side effects are handled directly by the VM's attribute
				and text-content opcodes — you rarely need to write an explicit effect for UI
				updates.
			</p>
			<section class="callout">
				<strong>Avoid circular effects</strong>
				<p>
					An effect that writes to a signal it also reads will loop. The runtime detects
					and breaks cycles after one iteration, but the pattern is a design smell.
					Use computed values when you need a derived write — they are evaluated lazily
					and do not re-trigger themselves.
				</p>
			</section>
		</section>
		...
		<section id="dependency-tracking">
			...
			<p>
				GoSX uses push-pull dependency tracking. Signals push change notifications to
				their subscribers. Computed values and effects pull their inputs when they
				re-evaluate, rebuilding the dependency edge set on each run. This means
...
summary := signal.Computed(func() string {
    ...
    return details.Get()
			...
			<p>
				The dependency graph is rebuilt from scratch on each evaluation. There is no
				bookkeeping required when you add or remove a branch — the tracking is automatic
				and correct for any control flow shape.
			</p>
			<div class="feature-grid">
				<div class="card">
					<strong>Automatic</strong>
					<p>No dependency arrays. No <span class="inline-code">watch()</span> calls. Just read signals inside a reactive scope.</p>
				</div>
				<div class="card">
					<strong>Precise</strong>
					<p>Only computeds and effects that actually read a changed signal are re-run.</p>
				</div>
				<div class="card">
					<strong>Conditional</strong>
					<p>Branches inside a computed are tracked correctly — unreachable signals are not subscribed.</p>
				</div>
				<div class="card">
					<strong>Cycle-safe</strong>
					<p>The runtime detects write-then-read cycles and stops them after one iteration.</p>
				</div>
			</div>
		</section>
		...
		<section id="batch-updates">
			...
			<p>
				When multiple signals change together — for example, when updating a form with
				several fields — you want all computeds and effects to re-run once, not once
				per signal write.
...
signal.Batch(func() {
    ...
    lastName.Set("Lovelace")
			...
			<p>
				...
				<span class="inline-code">signal.Batch</span>
				calls are merged into the outer batch. Propagation fires only when the outermost
				batch exits.
			</p>
			<p>
				The island VM applies batching automatically for event handlers. When a
				<span class="inline-code">data-on-click</span>
				expression writes multiple signals, the DOM is updated once after the entire
		...
		<section id="signal-store">
			...
			<p>
				For pages with many related signals, group them into a store — a plain struct
				with signal fields. A store is not a framework concept; it is a Go struct.
				Pass it to the template via the route loader's data map.
			</p>
...
func Load(ctx *route.RouteContext, page route.FilePage) (any, error) {
    ...
    return map[string]any{"cart": cart}, nil
			...
			<p>
				...
				in island markup resolves the computed value and subscribes the island program
				to its output. Updates to

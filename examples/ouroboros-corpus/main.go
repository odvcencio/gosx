...
func encodePrograms() (appPrograms, error) {
	...
	return appPrograms{counter: counter, tabs: tabs, toggle: toggle, shared: shared}, nil
}
...
func serveFixtureVideo(w http.ResponseWriter, r *http.Request) {
	...
	http.ServeContent(w, r, "ouroboros-placeholder.mp4", time.Unix(1700000000, 0), bytes.NewReader(data))
}
...
func runtimeRoot() string {
	root := repoRoot()
	if _, err := os.Stat(path.Join(root, "build", "bootstrap.js")); err == nil {
		return root
	}
	if _, err := os.Stat(path.Join(root, "build.json")); err == nil {
		return root
	}
	if _, err := os.Stat(path.Join(root, "dist", "build.json")); err == nil {

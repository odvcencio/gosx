  // GoSX browser text-layout engine.
  //
  ...
  var textLayoutRuntimeApi = window.__gosx_runtime_api || {};

  var setAttrValue = textLayoutRuntimeApi.setAttrValue || function() {};
  var setStyleValue = textLayoutRuntimeApi.setStyleValue || function() {};
  var walkElementTree = textLayoutRuntimeApi.walkElementTree || function() {};
  var textLayoutComputedStyle = textLayoutRuntimeApi.textLayoutComputedStyle || function() { return null; };
  var textLayoutComputedStyleValue = textLayoutRuntimeApi.textLayoutComputedStyleValue || function() { return ""; };
  var textLayoutNumberValue = textLayoutRuntimeApi.textLayoutNumberValue || function(value, fallback) {
    var parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
  };
  var textLayoutLengthValue = textLayoutRuntimeApi.textLayoutLengthValue || textLayoutNumberValue;
  var textLayoutLogicalInlineSize = textLayoutRuntimeApi.textLayoutLogicalInlineSize || function(width) {
    return Math.max(0, Number(width) || 0);
  };
  var normalizeTextLayoutWritingMode = textLayoutRuntimeApi.normalizeTextLayoutWritingMode || function() {
    return "horizontal-tb";
  };
  var normalizeTextLayoutOverflow = textLayoutRuntimeApi.normalizeTextLayoutOverflow || function(value) {
  ...
  // Follow-up: export scheduleVisualInvalidation and cancelInvalidation on
  // window.__gosx_runtime_api and delete this copy.
  var textLayoutVisualInvalidations = new Map();
  var textLayoutVisualInvalidationScheduled = false;

  function textLayoutMergedReason(previous, next) {
    if (!previous) {
      return next;
    }
  ...
  var gosxScheduleVisualInvalidation = typeof textLayoutRuntimeApi.scheduleVisualInvalidation === "function"
    ? textLayoutRuntimeApi.scheduleVisualInvalidation
    : function(key, reason, callback) {
      if (!key || typeof callback !== "function") {
        return;
      }
      var entry = textLayoutVisualInvalidations.get(key) || { callback: null, reason: "" };
      entry.callback = callback;
      entry.reason = textLayoutMergedReason(entry.reason, reason || "visual");
      textLayoutVisualInvalidations.set(key, entry);
      if (textLayoutVisualInvalidationScheduled) {
        return;
      }
      textLayoutVisualInvalidationScheduled = true;
      var flush = function(frameTime) {
        textLayoutVisualInvalidationScheduled = false;
        while (textLayoutVisualInvalidations.size > 0) {
          var pending = Array.from(textLayoutVisualInvalidations.values());
          textLayoutVisualInvalidations.clear();
      ...
      setTimeout(function() { flush(Date.now()); }, 0);
    };

  var gosxCancelInvalidation = typeof textLayoutRuntimeApi.cancelInvalidation === "function"
    ? textLayoutRuntimeApi.cancelInvalidation
    : function(key) {
      if (!key) {
        return;
      }
      textLayoutVisualInvalidations.delete(key);
    };
    ...
    if (record.config && record.config.observe) {
      ...
      if (window.__gosx.presentation && typeof window.__gosx.presentation.observe === "function") {
        record.stopPresentation = window.__gosx.presentation.observe(element, function() {
          ...
          // now runs in its own IIFE with its own scheduler, so scheduling here
          // would cost a second frame. refreshManagedTextLayoutRecord returns
          // early when the layout key is unchanged, so a duplicate is cheap.
  ...
  var textLayoutEngineAPI = {
    ...
    revision: gosxTextLayoutRevision,
    invalidate: invalidateTextLayoutCaches,
    layoutBrowserText: layoutBrowserText,
    applyPresentation: applyTextLayoutPresentation,
    onInvalidated: onTextLayoutInvalidated,
    adoptLayoutImpl: adoptTextLayoutImpl,
    adoptMetricsImpl: adoptTextLayoutMetricsImpl,
    adoptRangesImpl: adoptTextLayoutRangesImpl,
    observe: observeManagedTextLayout,
    mountAll: mountManagedTextLayouts,
    refreshAll: refreshManagedTextLayouts,
    dispose: disposeManagedTextLayout,
    disposeAll: disposeManagedTextLayouts,
    read: function(element) {
      var record = element ? textLayoutRecordsByElement.get(element) : null;
      return record ? record.result : null;
    },
  };

  window.__gosx_textlayout_api = textLayoutEngineAPI;
  if (typeof window.__gosx_register_text_layout_engine === "function") {
    window.__gosx_register_text_layout_engine(textLayoutEngineAPI);
  }

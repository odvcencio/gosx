   ...
   * @property {string|Uint8Array|number[]} packed
  ...
  function sceneUnpackIndices(src, count, bitWidth) {
    ...
    switch (bitWidth) {
      case 1:
        for (var i = 0; i < count; i++) {
          indices[i] = (Math.floor(src[Math.floor(i / 8)] / (1 << (i & 7)))) & 1;
        }
        break;
      case 2:
        for (var i = 0; i < count; i++) {
          indices[i] = (Math.floor(src[Math.floor(i / 4)] / (1 << ((i & 3) * 2)))) & 3;
        }
        break;
      case 4:
        for (var i = 0; i < count; i++) {
          indices[i] = (Math.floor(src[Math.floor(i / 2)] / (1 << ((i & 1) * 4)))) & 15;
        }
        break;
      case 8:
        for (var i = 0; i < count; i++) {
      ...
      default:
        ...
        for (var i = 0; i < count; i++) {
          ...
          for (var b = 0; b < bitWidth; b++) {
            if (src[Math.floor(bitPos / 8)] & (1 << (bitPos & 7))) {
              val |= 1 << b;
            }
            bitPos++;
          }

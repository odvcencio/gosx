  ...
  // and fitting each cascade's orthographic light-space box. Moved here from
  // 16-scene-webgl.js so the WebGPU backend can share the same PSSM fit
  // instead of re-deriving it; cascade parity between backends follows by
  // construction. Exposed via __gosx_scene3d_resource_api for unit tests;
  // each renderer's orchestration layer consumes the same helpers.

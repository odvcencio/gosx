...
var { readSceneMountSrc } = require("./runtime-test-harness.js");
var { readSceneRendererBackendSrc } = require("./scene3d-renderer-source-set.js");

// readSceneMountSrc joins every governed mount source (including the
// WebGL mount authority that owns computed-pose morphing);
// readSceneRendererBackendSrc("webgpu") joins the governed WebGPU renderer
// sources. Both are the shared test-harness helpers other renderer unit
// tests already use to read source text (see runtime-test-harness.js and
// scene3d-renderer-source-set.js).
var mountSrc = readSceneMountSrc();
var webgpuSrc = readSceneRendererBackendSrc("webgpu");
...
function makeSandbox() {
  var sandbox = {
    ...
    sceneAffineDeterminant: function () {
      return 1;
    },
    ...
    sceneModelTransform: function (v) {
    ...
    sceneObjectTransformNormal: function (model, v) {
      return { x: v.x, y: v.y, z: v.z };
    },
    sceneModelTransformTangents: function (tangents) {
      return new Float32Array(tangents);
    },
    sceneCloneModelMeshIndices: function (indices) {
      return indices || null;
    },
...
test("computed pose morph: CPU blend, prior copy semantics, and computedMorph identity", function () {
  ...
  // sourcePositions reflects the pose PRIOR to this event's blend, not the
  // in-place cache the blend just mutated (the repeated-blend bug).
  ...
  apply(object, source, target, model, 0.5);
  assert.equal(object.computedMorph, morphIdentity, "computedMorph object identity retained across events");
  ...
  // On the second event, sourcePositions must reflect the pose after the
  // first event's blend (0.225), not the original source (0.075) and not the
  // post-second-blend value (0.3).
  assert.ok(Math.abs(object.computedMorph.sourcePositions[1] - 0.225) < 1e-6);
  assert.ok(Math.abs(object.computedMorph.targetPositions[1] - 0.375) < 1e-6);
});

test("computed pose morph: missing target normals/tangents fall back to authored source, with prior semantics", function () {
  var sandbox = makeSandbox();
  var apply = sandbox.sceneComputedPoseApplyObjectMorph;
  var model = { id: "model" };
  var object = {

...
const budgets = [
  ...
  // 10-runtime-scene-core.js; cull-field validators in 15-scene-ir-schema-strict.ts;
  ...
  // Bumped raw 1_310_000 -> 1_311_000 for the G2 QualityLadder governor
  // (bidirectional work-based ABR: rung promote/demote, postEffects
  ...
  // v0.33.2: bumped brotli 282_000 -> 283_000 for the WebGPU persistent
  // frame-error resilience ladder (demote post-FX / fall back to WebGL),
  ...
  // 293_071.
  //
  // Bumped raw 1_362_000 -> 1_365_000, gzip 363_500 -> 366_500, brotli
  // 294_500 -> 297_000. Two independent feature sets landed in this bundle when
  // the audit branch merged main, so all three figures passed their ceiling.
  // Measured after the merge: 1_363_465 raw, 364_934 gzip, 295_439 brotli.
  //
  // NOTE for whoever reads this next: the assertions run raw first and stop at
  // the first failure, so a raw overage HIDES the compressed overages behind it.
  ...
  // on the merged bundle: 1_346_320 / 358_639 / 289_898. That measurement
  // predates the procedural point generator below (a parallel history off
  ...
  // the server. It buys far more than it costs: the m31labs.dev starfield
  // alone drops an 852_163-byte inline manifest to a ~300-byte descriptor,
  // and the saving scales with every deterministic point layer any app ships.
  // Merging the two histories (render truth + per-object pipeline validation
  // from main, procedural point clouds from this branch) puts all three
  // figures over either side's ceiling alone, so the ceiling below is set
  ...
  //
  // Merged main's render-truth telemetry and per-object WebGPU pipeline
  ...
  // note below). Exact merged measurement: 1_486_231 / 402_762 / 323_723.
  // Open post-init engine-factory registration plus the late-mount hook
  ...
  // O6 generated ABI, verified loader, mailbox and host-facade cutover. Exact
  // regenerated bytes; future changes ratchet against this authority build.
  // Bumped raw 1_522_281 -> 1_522_287 for the O-series propagation merge
  ...
  //
  // Bumped raw 1_526_695 -> 1_526_702 for the bootstrap-src TypeScript
  ...
  // Bumped raw 141_596 -> 141_682, gzip 38_297 -> 38_310, brotli 33_462 ->
  ...
  // Bumped raw 105_536 -> 105_622, gzip 27_963 -> 27_965 for the O-series
  ...
  // Bumped raw 712_000 -> 714_000 for the G2 QualityLadder governor and G1's
  // live-patchable postFXMaxPixels escape hatch (same delta as the
  ...
  // persistent-buffer upload path. Measured: 249_505 / 69_254 / 58_849;
  // retain narrow rounding headroom so later growth remains visible.
  { file: "bootstrap-feature-scene3d-webgl.js", raw: 249_600, gzip: 69_300, brotli: 58_900 },
  ...
  //
  // One entry, both histories. The audit branch ratcheted this chunk DOWN when
  ...
  //
  // Merged with render-truth telemetry and per-object WebGPU pipeline
  // validation. On the pre-split monolith those two features cost this same
  // scene3d bundle raw 723_000 -> 740_000 (15a's shared render-truth helpers,
  // 16-scene-webgl.js's per-effect chain marking and mesh/point
  // submitted-versus-drawn counters, 20-scene-mount.js's single
  // backend-truth JSON record, and 16b-scene-compute.js's
  // sceneShaderModuleError reading getCompilationInfo per module instead of
  // a device-global error scope). After this branch's per-feature split,
  // 15a and 16-scene-webgl.js ship in bootstrap-feature-scene3d-webgl.js
  // (below) and sceneShaderModuleError ships in
  ...
  { file: "bootstrap-feature-scene3d-command.js", raw: 13_900, gzip: 3_700, brotli: 3_250 },
  ...
  //
  // Merged with main's render-truth telemetry on the WebGPU path: the
  // post-chain dispatch record marked at fullscreenPass, explicit
  // pending/failed reporting for customPost, getCompilationInfo capture on
  // all nine authored Selena shader modules, the uncapturederror listener
  // and the device-loss journal entry. Main measured this addition alone at
  // 361_210 / 84_519 / 70_450 (raw 359_000 -> 364_000) against a base without
  ...
  { file: "bootstrap-feature-scene3d-animation.js", raw: 17_050, gzip: 6_800, brotli: 6_050 },
...
const routeBudgets = [
  {
    ...
    //
    // Merged with main's FINDING-001 fix: the motion reveal guard now compares
    // against a per-page-load token instead of a plain boolean
    // (GOSX_MOTION_LIFE_TOKEN), so a stale value from a different page life
    // can no longer suppress the reveal. Main measured this fix alone (against
    ...
    //
    // Bumped raw 251_469 -> 251_555, gzip 70_815 -> 70_828, brotli 62_277 ->
  ...
  // so maxMonolithFraction stays unset for them.
  {
    ...
    //
    // Bumped raw 1_163_000 -> 1_165_000, gzip 308_000 -> 310_500, brotli
    ...
    //
    // Merged with main's render-truth telemetry and per-object WebGPU
    // pipeline validation (see the bootstrap-feature-scene3d.js and
  ...
  },
  {
    ...
    // 222_915.
    //
    // Bumped for the audit-branch merge with main, set from measurement across
    // all five chunks in this route: 951_022 raw, 264_047 gzip, 224_313 brotli.
    ...
    //
    // Merged with main's render-truth telemetry (this route carries 15a and
  ...
  },
  {
    ...
    //
    // Bumped for the audit-branch merge with main, measured across all six chunks
  ...
  },
  {
    ...
    //
    // Bumped for the audit-branch merge with main, measured across the three
    ...
    //
    // Bumped raw 1_057_692 -> 1_057_706, gzip 278_510 -> 278_511, brotli
...
test("device-capability gate stays DRY (gosxLowEndHardware, AND-form, no inlined OR drift)", () => {
  ...
  // bootstrap-src is a mixed directory: sources that parse standalone are
  // TypeScript, while the concatenation fragments (prefix/suffix wrapper pairs)
  // must stay JavaScript because each .ts source is transpiled on its own.
  // Scan both extensions or the counts below silently undercount.
  const scanDirs = [
    { dir: srcDir, exts: [".js", ".ts"] },
  ...
  const env = fs.readFileSync(path.join(srcDir, "05-document-env.ts"), "utf8");
  assert.ok(

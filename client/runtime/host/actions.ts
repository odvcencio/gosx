...
//
// This source is intentionally a script rather than an ES module. The
// bootstrap builder concatenates it into the owning runtime chunks, while the
// host boundary itself stays typed and browser-owned.
 ...
 * @property {GoSXActionResult} result
 */
//
...
(function () {
    ...
    return gosxActionRequest(url, opts)
      .then(function (r) {
        ...
        return responsePayload(r).then(function (result) {
          ...
          if (!r.ok && !isHandledActionResult(r, result)) {
  ...
  function isHandledActionResult(response, result) {
    var status = Number(response && response.status) || 0;
    if (status < 400 || status >= 500 || !result || typeof result !== "object") return false;
    if (result.ok === false) return true;
    if (result.fieldErrors && typeof result.fieldErrors === "object") return true;
    if (typeof result.message === "string" && result.message) return true;
    if (result.data && typeof result.data === "object") {
      if (typeof result.data.html === "string") return true;
      if (Object.prototype.hasOwnProperty.call(result.data, "value")) return true;
    }
    return false;
  }

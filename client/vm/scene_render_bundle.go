...
// planeQuadIndices selects the XZ ring out of the box vertex list. boxVertices
// packs the -Z face first and the +Z face second, so with height 0 the first
// four entries collapse onto ONE edge (index 2 duplicates 1, index 3
// duplicates 0) and every plane quad has zero area. Indices 0, 1, 5, 4 walk
// the real ring: (-x,-z) -> (+x,-z) -> (+x,+z) -> (-x,+z). This mirrors
// planeQuadVertices in client/js/bootstrap-src/12-scene-geometry.js and the
// native reference in render/bundle/primitive.go planeGeometry.

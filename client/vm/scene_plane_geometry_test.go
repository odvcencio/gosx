...
// triangleArea returns the area of a triangle in world space.
...
func distinctPoints(points []point3) int {
	...
	for _, p := range points {
		// Normalise negative zero so -0 and +0 are the same corner.
		seen[[3]float64{p.X + 0, p.Y + 0, p.Z + 0}] = struct{}{}
	}
	return len(seen)
}

// TestBoxVertexSliceIsDegenerate documents the trap the plane generators must
// avoid. boxVertices packs the -Z face first, so at height 0 the first four
// entries collapse onto one edge.
...
func TestPlaneQuadVerticesWalksTheRing(t *testing.T) {
	...
	for i, w := range want {
		if math.Abs(quad[i].X-w[0]) > 1e-9 || math.Abs(quad[i].Y-w[1]) > 1e-9 || math.Abs(quad[i].Z-w[2]) > 1e-9 {
			t.Fatalf("corner %d = %+v, want %v", i, quad[i], w)
		}
	}
}

// TestScenePlaneSurfaceCornersArea is the regression guard for the defect that
// made every textured plane and every HTML-texture 3D surface invisible: the
// quad had zero area, so the rasterizer had nothing to fill and no error was
// reported anywhere.
func TestScenePlaneSurfaceCornersArea(t *testing.T) {
	...
	for _, tc := range cases {
		object := sceneObject{Kind: "plane", Width: tc.width, Depth: tc.depth, ScaleX: 1, ScaleY: 1, ScaleZ: 1}
		corners := scenePlaneSurfaceCorners(object, motion.Quat{X: 0, Y: 0, Z: 0, W: 1}, clipTRS{}, 0)
		if len(corners) != 4 {
		...
		area := triangleArea(corners[0], corners[1], corners[2]) + triangleArea(corners[0], corners[2], corners[3])

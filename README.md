...
- **Lights** — `AmbientLight`, `DirectionalLight`, `PointLight`, `SpotLight`, `HemisphereLight`, `RectAreaLight`, `LightProbe`; directional, spot and point shadow maps on WebGL2 and WebGPU, sharing two shadow-light slots; per-light `CastShadow`/`ShadowBias`/`ShadowSize`/`ShadowSoftness` plus a scene-wide `Shadows.MaxPixels` cap; point lights render into a single 3x2 depth atlas with six face passes per light at a 6*S*S budget cost

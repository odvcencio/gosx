#!/usr/bin/env node
//
// windows-water-probe.mjs -- the /demos/water regression harness.
//
// This was scripts/windows-scene3d-probe.mjs until the render-truth work split
// the two roles apart. It is a water-demo harness, not a general Scene3D probe:
// roughly 45 of its assertions name water-specific telemetry (caustics passes,
// pool tile textures, duck mesh selection, a 50fps floor), so it fails
// immediately on any other page for reasons unrelated to that page.
//
// Kept verbatim because it is a good harness for what it actually covers. For
// a general render-truth probe on an arbitrary URL, use
// scripts/windows-scene3d-probe.mjs, which reaches this file via --water.
//
// Invocation is unchanged:
//   node scripts/windows-water-probe.mjs --url <water URL> --browser
//

...
// TestDiffSceneRejectsRemountOnlyBeforeMutation proves a remount-only diff is
// refused before it can create history or wake a watcher.
func TestDiffSceneRejectsRemountOnlyBeforeMutation(t *testing.T) {
	...
	if commands != nil {
		t.Fatalf("remount-only diff commands = %v, want nil", commands)
	}
...
// TestDiffSceneRejectsMixedDiffAtomically proves a supported command cannot be
// committed when the same diff also needs a full scene remount.
func TestDiffSceneRejectsMixedDiffAtomically(t *testing.T) {
	doc, bound := newBoundDoc(t)
	base := scene.SceneIR{Objects: []scene.ObjectIR{{ID: "cube", Kind: "box"}}}
	...
	next := base
	next.Objects = append([]scene.ObjectIR(nil), base.Objects...)
	next.Objects[0].X = 7
	...
	if commands != nil {
		t.Fatalf("mixed diff commands = %v, want nil on rejection", commands)
	}
	if hash != (crdt.ChangeHash{}) {
...
// TestDiffSceneRemountFieldsAreStable proves every remount field is reported
// in the sorted order scene.DiffScene promises to its callers.
...
// TestDiffScenePreservesOrdinaryAndEmptyBehavior proves a supported diff still
// commits its command stream, while an empty diff remains a no-op.
func TestDiffScenePreservesOrdinaryAndEmptyBehavior(t *testing.T) {
	doc, bound := newBoundDoc(t)
	var seen []scene.Command
	...
	next.Objects = append([]scene.ObjectIR(nil), base.Objects...)
	next.Objects[0].X = 4
	commands, hash, err := bound.DiffScene(base, next, "patch")
	if err != nil {
		t.Fatal(err)
	}
	if hash == (crdt.ChangeHash{}) {
		t.Fatal("ordinary diff returned a zero hash")
	}
	assertCommandsEqual(t, "ordinary diff", commands, seen)
	...
	seen = nil
	commands, hash, err = bound.DiffScene(next, next, "empty")
	if err != nil {
		t.Fatal(err)
	}
	if commands != nil {
		t.Fatalf("empty diff commands = %v, want nil", commands)
	}
	if hash != (crdt.ChangeHash{}) {
	...
	if !bytes.Equal(before, after) {
		t.Fatal("empty diff changed the document snapshot")
	}
}
